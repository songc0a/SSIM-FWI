...
class Misfit_envelope(Misfit):
    ...
    def __init__(self, dt: float = 1, p: float = 1.5, instaneous_phase: bool = False,) -> None:
        super().__init__()
        self.p = p
    ...
    def forward(self, obs: torch.Tensor, syn: torch.Tensor) -> torch.Tensor:
        ...
        rsd_obs = torch.zeros((obs.shape[0], obs.shape[2], obs.shape[1]), device=device)  # Residual storage
        rsd_pre = torch.zeros((obs.shape[0], obs.shape[2], obs.shape[1]), device=device)  # Residual storage
        ...
        for ishot in range(obs.shape[0]):
            ...
            if self.instaneous_phase:
                ...
                rsd_obs[ishot,trace_idx,:] = (envelopes_obs**self.p).unsqueeze(0)
                rsd_pre[ishot,trace_idx,:] = (envelopes_syn**self.p).unsqueeze(0)

  
        loss = 0.5 * torch.sum(rsd * rsd * self.dt)  # L2 loss with time weighting
        # loss = self.criterion(rsd_obs,rsd_pre)
